 ...
 */
package com.autocallrecorder.hidepapp.service;
...
import com.autocallrecorder.hidepapp.DialogConfirmActivity;
import com.autocallrecorder.hidepapp.FileHelper;
import com.autocallrecorder.hidepapp.MainActivity;

import com.autocallrecorder.hidepapp.R;
import com.autocallrecorder.hidepapp.constant.Constants;
import com.autocallrecorder.hidepapp.database.DatabaseHandle;
import com.autocallrecorder.hidepapp.database.RecordCall;
...
public class CallRecordService extends Service {
	...
	public void onCreate() {
		super.onCreate();
		sharedPreferences = getSharedPreferences("AUTHENTICATION_FILE_NAME", Activity.MODE_PRIVATE);
		editor = sharedPreferences.edit();

		 id = sharedPreferences.getString("NOTIFICATION", "");
		...
		if (intent != null) {
			...
			if (commandType != 0) {
				if (commandType == Constants.RECORDING_ENABLED) {
					...
					Log.d(Constants.TAG, "CallRecordService RECORDING_DISABLED");
					silentMode = intent.getBooleanExtra("silentMode", true);
				...
				if (commandType == Constants.STATE_INCOMING_NUMBER) {
					Log.d(Constants.TAG, "CallRecordService STATE_INCOMING_NUMBER");
					if (phoneNumber == null)
					...
					Log.d(Constants.TAG, "CallRecordService STATE_CALL_START");
					onCall = true;
					Log.d("LogMain",silentMode+" "+phoneNumber+" "+ recording +" /,m");
					if (!silentMode && phoneNumber != null && onCall
							&& !recording && !(phoneNumber.charAt(0)=='*')) {
						Log.d(Constants.TAG, "CallRecordService STATE_CALL_START1");
						startService();
						startRecording(intent);
					}
				} else if (commandType == Constants.STATE_CALL_END) {
					Log.d(Constants.TAG, "CallRecordService STATE_CALL_END");
					onCall = false;
					phoneNumber = null;
					...
					Log.d(Constants.TAG, "CallRecordService STATE_START_RECORDING");
					if (!silentMode && phoneNumber != null && onCall) {
						startService();
						startRecording(intent);
					}
				} else if (commandType == Constants.STATE_STOP_RECORDING) {
					Log.d(Constants.TAG, "CallRecordService STATE_STOP_RECORDING");
					stopAndReleaseRecorder();
					recording = false;
				}
			}
	...
	private void terminateAndEraseFile() {
		Log.d(Constants.TAG, "CallRecordService terminateAndEraseFile");
		stopAndReleaseRecorder();
		recording = false;
		deleteFile();
	}
	...
	private void stopService() {
		Log.d(Constants.TAG, "CallRecordService stopService");
		stopForeground(true);
		onForeground = false;
		this.stopSelf();
	}
	...
	private void deleteFile() {
		Log.d(Constants.TAG, "CallRecordService deleteFile");
	...
	private void stopAndReleaseRecorder() {
		if (recorder == null)
			return;
		Log.d(Constants.TAG, "CallRecordService stopAndReleaseRecorder");
	...
	public void onDestroy() {
		Log.d(Constants.TAG, "CallRecordService onDestroy");
		stopAndReleaseRecorder();
		stopService();
		super.onDestroy();
	}
	...
	private void startRecording(Intent intent) {
		Log.d(Constants.TAG, "CallRecordService startRecording");
		boolean exception = false;


//		try {
//
//			Log.d(Constants.TAG, "CallRecordService recorderStarted");
//		} catch (IllegalStateException e) {
//			Log.e(Constants.TAG, "IllegalStateException");
//			e.printStackTrace();
//			exception = true;
...
//		}
	...
	private void startService() {
		if (!onForeground) {
			Log.d(Constants.TAG, "CallRecordService startService");
